...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listfoireauxquestions.filter(contenu => contenu.categorie === 'foire-aux-questions') : listfoireauxquestions.filter(contenu => contenu.categorie === 'foire-aux-questions' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listfoireauxquestions.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listfoireauxquestions: 'foireauxquestions/listfoireauxquestions',
      headers: 'foireauxquestions/headerfoireauxquestions',
    ...
    methods: {
      ...
      editItem (item) {   
        this.$store.dispatch('contenus/getDetail',item)
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "foireauxquestions"}});
      },
       deleteItem (item) {
        this.dialog=false   

...
<v-data-table
  ...
  :headers="headers"
  :items="tab=='tout'?listmonnaies : listmonnaies.filter(monnaie => monnaie.status === tab)"

  item-key="id"
  items-per-page="5"

  class="flat pt-4"
  :loading="listmonnaies.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      ...
      headers: 'monnaies/headermonnaies'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('monnaies/getDetail',item)
        this.$router.push('/monnaies/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('monnaies/getDetail',item)
        this.$router.push('/monnaies/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$siratApi.$delete('/monnaies/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('monnaies/deletemonnaie',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let monnaie = this.selected.map(function(value){ return value})[0]
          this.$store.commit('monnaies/initdetail',monnaie)
          this.$router.push('/monnaies/detail/'+monnaie.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez sélectionner un élément')
          ...
          let monnaie = this.selected.map(function(value){ return value})[0]
          this.$store.commit('monnaies/initdetail',monnaie)
          this.$router.push('/monnaies/modifier/'+monnaie.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)

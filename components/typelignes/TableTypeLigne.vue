...
<v-data-table
  v-model="selected"
  :headers="headers"
  :items="tab=='tout'?listtypelignes : listtypelignes.filter(typeligne => typeligne.status === tab)"
  :single-select="singleSelect"
  item-key="id"
  items-per-page="5"
  show-select
  class="flat pt-4"
  :loading="listtypelignes.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      listtypelignes: 'typelignes/listtypelignes',
      headers: 'typelignes/headertypelignes'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('typelignes/getDetail',item)
        this.$router.push('/typelignes/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('typelignes/getDetail',item)
        this.$router.push('/typelignes/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$siratApi.$delete('/type_lignes/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('typelignes/deletetypeligne',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let typeligne = this.selected.map(function(value){ return value})[0]
          this.$store.commit('typelignes/initdetail',typeligne)
          this.$router.push('/typelignes/detail/'+typeligne.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez sélectionner un élément')
          ...
          let typeligne = this.selected.map(function(value){ return value})[0]
          this.$store.commit('typelignes/initdetail',typeligne)
          this.$router.push('/typelignes/modifier/'+typeligne.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)

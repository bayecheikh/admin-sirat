  ...
  export default {
    ...
    methods: {
      ...
      getList(){
        this.page=1
        this.progress=true
        this.$store.dispatch('usagers/getDataSearch',null)
        this.$essApi.$get('/list-users?page=1')
        .then(async (response) => {
      ...
      getResult(page,param){
         ...
         this.$essApi.get('/usager-multiple-search/'+param+'?page='+page)
          .then(async (response) => {
            ...
            let totalPages = Math.ceil(response.data.meta.total / response.data.meta.per_page)
            this.$store.dispatch('usagers/getTotalPage',totalPages)
            this.$store.dispatch('usagers/getPerPage',response.data.meta.per_page)

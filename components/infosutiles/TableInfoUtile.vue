...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listinfosutiles.filter(contenu => contenu.categorie === 'informations-utiles') : listinfosutiles.filter(contenu => contenu.categorie === 'informations-utiles' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listinfosutiles.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listinfosutiles: 'infosutiles/listinfosutiles',
      headers: 'infosutiles/headerinfosutiles',
    ...
    methods: {
      ...
      editItem (item) {   
        this.$store.dispatch('contenus/getDetail',item)
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "infosutiles"}});
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      modifier(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "infosutiles"}});
        }
      },
      supprimer(){
        if(this.selected.length>=1)

...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listtarifspeages.filter(contenu => contenu.categorie === 'tarifs-peages') : listtarifspeages.filter(contenu => contenu.categorie === 'tarifs-peages' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listtarifspeages.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listtarifspeages: 'tarifspeages/listtarifspeages',
      headers: 'tarifspeages/headertarifspeages',
    ...
    methods: {
      ...
      editItem (item) {   
        this.$store.dispatch('contenus/getDetail',item)
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "tarifspeages"}});
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      modifier(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "tarifspeages"}});
        }
      },
      supprimer(){
        if(this.selected.length>=1)

...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listpostepeages : listpostepeages.filter(postepeage => postepeage.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listpostepeages.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      ...
      headers: 'postepeages/headerpostepeages',
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('postepeages/getDetail',item)
        this.$router.push('/postepeages/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('postepeages/getDetail',item)
        this.$router.push('/postepeages/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$siratApi.$delete('/postepeages/'+this.activeItem.id)
        .then(async (response) => { 
            console.log('Réponse delete ++++++: ', response)
            this.$store.dispatch('postepeages/deletepostepeage',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let postepeage = this.selected.map(function(value){ return value})[0]
          this.$store.commit('postepeages/initdetail',postepeage)
          this.$router.push('/postepeages/detail/'+postepeage.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez sélectionner un élément')
          ...
          let postepeage = this.selected.map(function(value){ return value})[0]
          this.$store.commit('postepeages/initdetail',postepeage)
          this.$router.push('/postepeages/modifier/'+postepeage.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)

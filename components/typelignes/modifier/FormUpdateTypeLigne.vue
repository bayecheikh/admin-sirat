...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    mounted: function() {
      this.model.id = this.detailtypeligne.id
      this.model.libelle = this.detailtypeligne.libelle
    },
    computed: mapGetters({
      detailtypeligne:'typelignes/detailtypeligne'
    }),
    data: () => ({
      loading: false,
      message:null,
    ...
    methods: {
      submitForm () {
        ...
        validation && this.$msasApi.put('/type_lignes/'+this.model.id, {...this.model})
          .then((res) => {    
            this.$store.dispatch('toast/getMessage',{type:'success',text:res.data.message || 'Ajout réussi'})
            this.$router.push('/typelignes');
          })
          .catch((error) => {

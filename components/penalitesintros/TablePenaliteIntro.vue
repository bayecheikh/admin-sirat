...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listpenalitesintros.filter(contenu => contenu.categorie === 'penalites-intro') : listpenalitesintros.filter(contenu => contenu.categorie === 'penalites-intro' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listpenalitesintros.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listpenalitesintros: 'penalitesintros/listpenalitesintros',
      headers: 'penalitesintros/headerpenalitesintros',
    ...
    methods: {
      ...
      editItem (item) {   
        this.$store.dispatch('contenus/getDetail',item)      
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "penalitesintros"}});
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      modifier(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "penalitesintros"}});
        }
      },
      supprimer(){
        if(this.selected.length>=1)

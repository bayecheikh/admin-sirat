...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listactivites.filter(contenu => contenu.categorie === 'activites') : listactivites.filter(contenu => contenu.categorie === 'activites' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    methods: {
      ...
      editItem (item) {   
        this.$store.dispatch('contenus/getDetail',item)
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "activites"}});
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      modifier(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "activites"}});
        }
      },
      supprimer(){
        if(this.selected.length>=1)

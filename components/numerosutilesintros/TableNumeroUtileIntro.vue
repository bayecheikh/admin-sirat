...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listnumerosutilesintros.filter(contenu => contenu.categorie === 'numeros-utiles-intro') : listnumerosutilesintros.filter(contenu => contenu.categorie === 'numeros-utiles-intro' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listnumerosutilesintros.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listnumerosutilesintros: 'numerosutilesintros/listnumerosutilesintros',
      headers: 'numerosutilesintros/headernumerosutilesintros',
    ...
    methods: {
      ...
      editItem (item) {   
        this.$store.dispatch('contenus/getDetail',item)
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "numerosutilesintros"}});
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      modifier(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "numerosutilesintros"}});
        }
      },
      supprimer(){
        if(this.selected.length>=1)

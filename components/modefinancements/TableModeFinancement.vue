...
<v-data-table
  ...
  :headers="headers"
  :items="tab=='tout'?listmodefinancements : listmodefinancements.filter(modefinancement => modefinancement.status === tab)"
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('modefinancements/getDetail',item)
        this.$router.push('/modefinancements/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('modefinancements/getDetail',item)
        this.$router.push('/modefinancements/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$siratApi.$delete('/mode_financements/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('modefinancements/deletemodefinancement',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let modefinancement = this.selected.map(function(value){ return value})[0]
          this.$store.commit('modefinancements/initdetail',modefinancement)
          this.$router.push('/modefinancements/detail/'+modefinancement.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez sélectionner un élément')
          ...
          let modefinancement = this.selected.map(function(value){ return value})[0]
          this.$store.commit('modefinancements/initdetail',modefinancement)
          this.$router.push('/modefinancements/modifier/'+modefinancement.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)

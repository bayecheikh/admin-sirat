...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listavisconcurrences.filter(contenu => contenu.categorie === 'avis-d-appel-a-concurence') : listavisconcurrences.filter(contenu => contenu.categorie === 'avis-d-appel-a-concurence' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listavisconcurrences.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listavisconcurrences: 'avisconcurrences/listavisconcurrences',
      headers: 'avisconcurrences/headeravisconcurrences',
    ...
    metaInfo () {
      ...
      visualiserItem (item) {   
        ...
        this.$router.push('/marchespublics/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('marchespublics/getDetail',item)
        this.$router.push({path: '/marchespublics/modifier/'+item.id, query: {categorie_slug: "avis-d-appel-a-concurence"}});
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$siratApi.$delete('/marchepublics/'+this.activeItem.id)
        .then(async (response) => { 
            console.log('Réponse delete ++++++: ', response)
            this.$store.dispatch('marchespublics/deletemarchepublic',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          this.$store.commit('marchespublics/initdetail',marchepublic)
          this.$router.push('/marchespublics/detail/'+marchepublic.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez sélectionner un élément')
          ...
          let marchepublic = this.selected.map(function(value){ return value})[0]
          this.$store.commit('marchespublics/initdetail',marchepublic)
          this.$router.push('/marchespublics/modifier/'+marchepublic.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)

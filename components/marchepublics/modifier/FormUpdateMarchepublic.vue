...
import {
  ...
  export default {
    ...
    computed: mapGetters({
      detailmarchepublic:'marchepublics/detailmarchepublic',
    ...
    methods: {
      getDetail(id){
          this.progress=true
          this.$msasApi.$get('/marchepublics/'+id)
        .then(async (response) => {
            console.log('Detail marchepublic ++++++++++',response.data)
            this.$store.dispatch('marchepublics/getDetail',response.data)
      ...
      submitForm () {
        ...
        validation && this.$msasFileApi.post('/marchepublics/'+this.model.id, formData)
          .then((res) => {    
            this.$store.dispatch('toast/getMessage',{type:'success',text:res.data.message || 'Mofication réussie'})
            this.$router.push('/marchepublics');

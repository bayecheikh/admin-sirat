...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?liststages.filter(contenu => contenu.categorie === 'stages') : liststages.filter(contenu => contenu.categorie === 'stages' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="liststages.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      liststages: 'stages/liststages',
      headers: 'stages/headerstages',
    ...
    methods: {
      ...
      editItem (item) {   
        this.$store.dispatch('gestionrhs/getDetail',item)
        this.$router.push({path: '/gestionrhs/modifier/'+item.id, query: {categorie_slug: "stages"}});
      },
       deleteItem (item) {
        this.dialog=false   

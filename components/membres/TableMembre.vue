...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listmembres : listmembres.filter(membre => membre.status === tab)"
  item-key="id"
  items-per-page="5"
  class="flat pt-4"
  :loading="listmembres.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    computed: mapGetters({
      listmembres: 'membres/listmembres',
      headers: 'membres/headermembres'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      visualiserItem (item) {   
        this.$store.dispatch('membres/getDetail',item)
        this.$router.push('/membres/detail/'+item.id);
      },
      editItem (item) {   
        this.$store.dispatch('membres/getDetail',item)
        this.$router.push('/membres/modifier/'+item.id);
      },
       deleteItem (item) {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'}) 
        this.$msasApi.$delete('/membres/'+this.activeItem.id)
        .then(async (response) => { 
            this.$store.dispatch('membres/deletemembre',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          let membre = this.selected.map(function(value){ return value})[0]
          this.$store.commit('membres/initdetail',membre)
          this.$router.push('/membres/detail/'+membre.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let membre = this.selected.map(function(value){ return value})[0]
          this.$store.commit('membres/initdetail',membre)
          this.$router.push('/membres/modifier/'+membre.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)

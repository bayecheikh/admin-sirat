...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listpostepeagesintros.filter(contenu => contenu.categorie === 'postes-de-peages-intro') : listpostepeagesintros.filter(contenu => contenu.categorie === 'postes-de-peages-intro' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listpostepeagesintros.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listpostepeagesintros: 'postepeagesintros/listpostepeagesintros',
      headers: 'postepeagesintros/headerpostepeagesintros'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      ...
      editItem (item) {   
        ...
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "postepeagesintros"}});
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      visualiser(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "postepeagesintros"}});
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push('/contenus/modifier/'+contenu.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)

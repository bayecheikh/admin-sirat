...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listmissionsintros.filter(contenu => contenu.categorie === 'mission-intro') : listmissionsintros.filter(contenu => contenu.categorie === 'mission-intro' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listmissionsintros.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listmissionsintros: 'missionsintros/listmissionsintros',
      headers: 'missionsintros/headermissionsintros',
    ...
    methods: {
      ...
      editItem (item) {   
        this.$store.dispatch('contenus/getDetail',item)      
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "missionsintros"}});
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      modifier(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "missionsintros"}});
        }
      },
      supprimer(){
        if(this.selected.length>=1)

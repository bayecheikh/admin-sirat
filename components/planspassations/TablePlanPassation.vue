<template>
  ...
  <v-data-table
    :headers="headers"
    :items="tab=='tout'?listplanspassations.filter(contenu => contenu.categorie === 'plan-de-passation') : listplanspassations.filter(contenu => contenu.categorie === 'plan-de-passation' && contenu.status === tab)"
    item-key="id"
    items-per-page="20"
    class="flat pt-4"
    :loading="listplanspassations.length?false:true" 
  ...
  import { mapMutations, mapGetters } from 'vuex'
    export default {
      ...
      computed: mapGetters({
        listplanspassations: 'planspassations/listplanspassations',
        headers: 'planspassations/headerplanspassations',
      ...
      metaInfo () {
        ...
        editItem (item) {   
          this.$store.dispatch('marchespublics/getDetail',item)
          this.$router.push({path: '/marchespublics/modifier/'+item.id, query: {categorie_slug: "plan-de-passation"}});
        },
         deleteItem (item) {
          this.dialog=false   

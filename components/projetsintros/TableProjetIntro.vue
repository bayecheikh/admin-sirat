...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listprojetsintros.filter(contenu => contenu.categorie === 'projets-intro') : listprojetsintros.filter(contenu => contenu.categorie === 'projets-intro' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listprojetsintros.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listprojetsintros: 'projetsintros/listprojetsintros',
      headers: 'projetsintros/headerprojetsintros'
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      ...
      editItem (item) {   
        ...
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "projetsintros"}});
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      visualiser(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "projetsintros"}});
        }
      },
      modifier(){
        if(this.selected.length!=1)

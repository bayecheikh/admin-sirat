...
<v-data-table
  :headers="headers"
  :items="tab=='tout'?listoffresemploisintros.filter(contenu => contenu.categorie === 'offre-emploi-intro') : listoffresemploisintros.filter(contenu => contenu.categorie === 'offre-emploi-intro' && contenu.status === tab)"
  item-key="id"
  items-per-page="20"
  class="flat pt-4"
  :loading="listoffresemploisintros.length?false:true" 
...
import { mapMutations, mapGetters } from 'vuex'
  export default {
    ...
    computed: mapGetters({
      listoffresemploisintros: 'offresemploisintros/listoffresemploisintros',
      headers: 'offresemploisintros/headeroffresemploisintros',
    ...
    methods: {
      ...
      editItem (item) {   
        this.$store.dispatch('contenus/getDetail',item)      
        this.$router.push({path: '/contenus/modifier/'+item.id, query: {categorie_href: "offresemploisintros"}});
      },
       deleteItem (item) {
        this.dialog=false   
      ...
      modifier(){
        ...
        alert('Veuillez sélectionner un élément')
          ...
          let contenu = this.selected.map(function(value){ return value})[0]
          this.$store.commit('contenus/initdetail',contenu)
          this.$router.push({path: '/contenus/modifier/'+contenu.id, query: {categorie_href: "offresemploisintros"}});
        }
      },
      supprimer(){
        if(this.selected.length>=1)

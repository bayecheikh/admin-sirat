<template>
  <div>
    ...
    <v-data-table
     v-model="selected"
      :headers="headers"
      :items="tab=='tout'?listAdherents.filter(user => user.roles[0] && user.roles[0].name != 'super_admin') : listAdherents.filter(user => user.status === tab)"
...
import RechercheAdherent from '@/components/adherents/RechercheAdherent';
  export default {
    components: {
      RechercheAdherent
    },
    ...
    computed: mapGetters({
      listAdherents: 'adherents/listadherents',
      headers: 'adherents/headeradherents',
      totalpage: 'adherents/totalpage',
      perpage: 'adherents/perpage',
      datasearch: 'adherents/datasearch',
    }),
    props: ['tab'],
    metaInfo () {
      return {
    ...
    methods: {
      getList(page){
        ...
        .then(async (response) => {
            console.log('Données reçu adherents ++++++: ', response.data.data)
            let totalPages = Math.ceil(response.data.total / response.data.per_page)
            this.$store.dispatch('adherents/getTotalPage',totalPages)
            this.$store.dispatch('adherents/getPerPage',response.data.per_page)
            this.$store.dispatch('adherents/getList',response.data.data)
        ...
        //console.log('total items++++++++++',this.paginationAdherent)
      },
       getResult(page,param){
         ...
         this.$msasApi.get('/user-multiple-search/'+param+'?page='+page)
          .then(async (response) => {
            ...
            await this.$store.dispatch('adherents/getList',response.data.data.data)
            let totalPages = Math.ceil(response.data.data.total / response.data.data.per_page)
            this.$store.dispatch('adherents/getTotalPage',totalPages)
            this.$store.dispatch('adherents/getPerPage',response.data.data.per_page)
      ...
      visualiserItem (item) {
        this.$store.dispatch('adherents/getDetail',item)
        this.$router.push('/adherents/detail/'+item.id);
      },
      editItem (item) {
        this.$store.dispatch('adherents/getDetail',item)
        this.$router.push('/adherents/modifier/'+item.id);
      },
      async deleteItem () {
        this.dialog=false   
        this.$store.dispatch('toast/getMessage',{type:'processing',text:'Traitement en cours ...'})  
        ...
        .then(async (response) => {             
            this.$store.dispatch('adherents/deleteAdherent',this.activeItem.id)
      ...
      visualiser(){
        ...
        alert('Veuillez selectionner un element')
          ...
          this.$store.commit('adherents/initdetail',user)
          this.$router.push('/adherents/detail/'+user.id);
        }
      },
      modifier(){
        if(this.selected.length!=1)
        alert('Veuillez selectionner un element')
          ...
          let user = this.selected.map(function(value){ return value})[0]
          this.$store.commit('adherents/initdetail',user)
          this.$router.push('/adherents/modifier/'+user.id);
        }
      },
      supprimer(){
        if(this.selected.length>=1)
